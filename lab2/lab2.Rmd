...
landfall <- tibble(Hurricane = c("Harvey", "Irma", "Jose", "Maria"), Date = c("August 25, 2017", "September 10, 2017", "Did not make landfall", "September 20, 2017" ))
...
ggplot(trends_pivot, aes(date, n)) +
	...
	geom_area(aes(fill = hurricane)) +
	geom_vline(
    aes(xintercept = Date), data = landfall, color = "grey91", 
    size = .6)
	
	
	
	geom_text(
    aes(x = Date, y = Hurricane, label = Hurricane), 
    data = landfall, 
